        ...
        uint16_t                CANbitRate)
{
    ...
    CAN_InitStruct.CAN_SJW = CAN_SJW_1tq/*CAN_SJW_4tq*/;     // changed by VJ, old value = CAN_SJW_1tq;
    ...
    if (result == 0)
    {
       printf("[LOG] Timeout error while initializing CAN bus\r\n");
    ...
    CAN_FilterInitStruct.CAN_FilterScale = CAN_FilterScale_16bit; //added changed to16bit
    ...
    NVIC_InitStructure.NVIC_IRQChannel = USB_LP_CAN1_RX0_IRQn; //added changed to STM32F10X_MD values
    NVIC_Init(&NVIC_InitStructure);
    NVIC_InitStructure.NVIC_IRQChannel = USB_HP_CAN1_TX_IRQn; //added changed to STM32F10X_MD values
    NVIC_Init(&NVIC_InitStructure);
...
void CO_CANverifyErrors(CO_CANmodule_t *CANmodule)
{
   ...
   if(CANmodule->errOld != err)
   {
      ...
      if(err & 0x02)
      {
        ...
        {
            printf("[ERROR] CAN TX Bus Passive\r\n");
      ...
      if(err & 0x01)
      {
        printf("[ERROR] CAN BUS RX/TX warning\r\n");
...
void CO_CANinterrupt_Rx(CO_CANmodule_t *CANmodule)
{
    ...
    // SESTODO: cast to (CanRxMsg*) is an ugly hack. Actually, CanRxMsg* and
    // CO_CANrxMsg_t are the same type, except that StdId member has been renamed
    // to ident. Cast is here to avoid incompatible pointer warning. Consider
    // refactoring!
    CAN_Receive(CANmodule->CANbaseAddress, CAN_FilterFIFO0, (CanRxMsg*) &CAN1_RxMsg);
